...
Clear[jfinal,ff1,chisqsnU,n,b,d,a,w,H0,chisqsnUbis,\[Delta]1bis,\[Delta]2bis,\[Delta]3bis,\[CapitalOmega]m,c,s,FF1,\[Delta]4bis,R,nc,ncbis,integralw,dzteor]
...
\[CapitalOmega]r=9.29*10^(-5);
aeq[j]:=\[CapitalOmega]r/\[CapitalOmega]m[j];

solutionz[z_]:=(\[CapitalOmega]r (1+z)^4 +  \[CapitalOmega]m[j] (1+z)^3+(1-\[CapitalOmega]m[j]-\[CapitalOmega]r) (1+z)^(3(1+w[j])))^(1/2);   (* Ecuaciones de Friedmann, calculan H(z)/H_0 *)
...
solutionhz[z_]:=67.3*(\[CapitalOmega]m[j] (1+z)^3+(1-\[CapitalOmega]m[j]) (1+z)^(3(1+w[j])) )^(1/2);  (* Ecuaciones de Friedmann, calcula el par\[AAcute]metro de Hubble para cada valor de \[CapitalOmega]m *)
...
solutionzcmb[z_]:=67.3*(\[CapitalOmega]r (1+z)^4 +  \[CapitalOmega]m[j] (1+z)^3+(1-\[CapitalOmega]m[j]-\[CapitalOmega]r) (1+z)^(3(1+w[j])))^(1/2);  (* Ecuaciones de Friedmann, calcula el par\[AAcute]metro de Hubble para cada valor de \[CapitalOmega]m *)
...
chi=chisnia+chiBAO+chihz+chiCMB;
...
\[Delta]4bis[j]=If[j>1,If[compchi>Aleatorio,w[j],\[Delta]4bis[j-1]],w[j]]; (* Para solo coger los mas peque\[NTilde]os significativamente *)
...
Print[{j,\[Delta]3bis[j],\[Delta]4bis[j],chisqsnUbis[j+1],nc[j],jj[nc[j]]}];

If[ncbis[j]>0||j==jfinal,ff1[nc[j],1]=N[jj[nc[j]]];ff1[nc[j],2]=N[chisqsnUbis[j]];ff1[nc[j],3]=N[\[Delta]3bis[j-1]];ff1[nc[j],4]=N[\[Delta]4bis[j-1]]];
...
FF1=Array[ff1,{jtotal,4}];
If[n==1,Export["AjustewCDM.dat",FF1],If[n==2,Export["Pruebaw001E1Bbis.dat",FF1],Export["Pruebaw001E1Cbis.dat",FF1]]];
Clear[FF1,ff1,q,l,m,b,\[CapitalOmega]m,s,\[Alpha],\[Delta]2bis,\[Delta]3bis,\[Delta]4bis,chisqsnU,chisqsnUbis,ncbis,nc,jtotal]
];


Do[ModuloMCMC2[10800,wx],{wx,1,1,1}](* Se suelen correr mas de 10000 *) 
